 ...
 *--------------------------------------------------------------------------------*/
	...
	render:function(){
		...
		ctrl_home.mainR.on('verPub',function(e){
			...
			window.open("/publicacion/" + e.context.uid,'_blank' );
		});
 ...
 /*		var volAvMin = 0
 		var volAvMax = 1
 		for (var i = 0; i < res.publicaciones.length; i++) {
 			console.log(res.publicaciones[i].info.cargamax,"CARGA MAX ")
 				if(res.publicaciones[i].info.cargamax>volAvMax) { volAvMax = res.publicaciones[i].info.cargamax }
 				if(res.publicaciones[i].info.cargamin<volAvMin) { volAvMin = res.publicaciones[i].info.cargamax }
 		}

 	 try{
          ctrl_home.sliderCap.noUiSlider.destroy();
            } catch (e) {} 

 		ctrl_home.sliderCap = document.getElementById('volFilter');
 		noUiSlider.create(ctrl_home.sliderCap, {
			start: [volAvMin, volAvMax],
			connect: true,
			margin : .5,
			step: .5,
			range: {
		...
		ctrl_home.sliderCap.noUiSlider.on('update', function(e){
			console.log(e)
			ctrl_home.mainR.set('volAvMin',e[0]);
			ctrl_home.mainR.set('volAvMax',e[1]);
			
		});


		ctrl_home.mainR.set('volAvMin',volAvMin);
		ctrl_home.mainR.set('volAvMax',volAvMax);



 		if(ctrl_home.mainR.get('masinfo')){
 			gGeo.init();
			gGeo.addPins([ctrl_home.selOrigen,ctrl_home.selDestino])
			ctrl_home.drawRoute();
 		}
*/
 	...
 	filterPubs : function(){
 		...
 		// Creación de Filtros
 		for (var i = 0; i < res.publicaciones.length; i++) {
 			for (var a = 0; a < res.publicaciones[i].info.carac.length; a++) {
 				var ext = JSON.search(caracAv,"//*[id="+ res.publicaciones[i].info.carac[a].id +"]")
 				if(ext.length==0) { 
 					res.publicaciones[i].info.carac[a].checked = true;
 					caracAv.push(res.publicaciones[i].info.carac[a])
 				}
 			}
 		...
 		// Servicios Extra
 		for (var i = 0; i < res.publicaciones.length; i++) {
 			for (var a = 0; a < res.publicaciones[i].info.servicios.length; a++) {
 				var ext = JSON.search(servAv,"//*[id="+ res.publicaciones[i].info.servicios[a].id +"]")
 				if(ext.length==0) { 
 					res.publicaciones[i].info.servicios[a].checked = true;
 					servAv.push(res.publicaciones[i].info.servicios[a])
 				}
 			}
 		...
 		//Años disponibles
 		for (var i = 0; i < res.publicaciones.length; i++) {
 				var ext = JSON.search(anosAv,"//*[desc="+ res.publicaciones[i].info.ano +"]")
 				if(ext.length==0) { 
 					anosAv.push({id:i,desc:res.publicaciones[i].info.ano,checked:true})
 			}
	...
	renderLogin(response){
		...
		console.log(event.context,"CONTEXTO CLICK")	
		...
		ctrl_home.loginR.on('openRegistro',function(){
			ctrl_home.modal = template.render('#registroT', '#modal', {tipoSel:2});
